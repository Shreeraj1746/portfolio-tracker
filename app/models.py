...
class Portfolio(Base):
    ...
    groups: Mapped[list[Group]] = relationship(back_populates="portfolio", cascade="all, delete-orphan")
    ...
    baskets: Mapped[list[Basket]] = relationship(back_populates="portfolio", cascade="all, delete-orphan")


class Group(Base):
    __tablename__ = "groups"
    __table_args__ = (UniqueConstraint("portfolio_id", "name", name="uq_group_portfolio_name"),)
...
class Asset(Base):
    ...
    basket_links: Mapped[list[BasketAsset]] = relationship(back_populates="asset", cascade="all, delete-orphan")
...
class Basket(Base):
    ...
    assets: Mapped[list[BasketAsset]] = relationship(back_populates="basket", cascade="all, delete-orphan")


class BasketAsset(Base):
    __tablename__ = "basket_assets"
    __table_args__ = (UniqueConstraint("basket_id", "asset_id", name="uq_basket_asset"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    basket_id: Mapped[int] = mapped_column(ForeignKey("baskets.id", ondelete="CASCADE"), nullable=False, index=True)
    asset_id: Mapped[int] = mapped_column(ForeignKey("assets.id", ondelete="CASCADE"), nullable=False, index=True)
